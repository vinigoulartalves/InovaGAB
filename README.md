...
| Login 401 | Conferir `.env` e contas `*@inovagab.local` (senhas `DEV_PASSWORD_*` do `setup-dev.sh`). Reinicie a API (`docker compose up -d --build api`) para **sincronizar** senhas demo no Mongo; ou `docker compose down -v` |
